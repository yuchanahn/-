...
use godot::{builtin::{math::{self, FloatExt}, StringName, Vector2}, engine::{input, utilities::{minf, move_toward, rand_from_seed}, AnimatedSprite2D, Node, Node2D, PackedScene}, log::godot_print, obj::Gd};

use crate::{animation_controller::{EAnim, PlayAnimationData}, col2d::{circle_cast, Box2D, Circle2D, Collision2D}, effect::Effect, game_manager::GameTick, player::*, random, utils::minus};

#[derive(Clone)]
pub struct WorldData {
    pub players: HashMap<u64, PlayerData>,
    pub collision: Vec<Collision2D>,
    pub hit_effect: Option<Gd<PackedScene>>,
}
...
pub struct PlayerData {
    ...
    pub guard_cooldown: f64,
    pub roll_start_dir: i32,
    pub health: f64,
...
fn is_anim_name(data: PlayerData, name: &str) -> bool {
    data.anim_data.as_ref().unwrap().name == name.into()
}
...
fn simulate_player(mut data: PlayerData, col_world: &Vec<Collision2D>, input: u8, tick: u64, delta: f64) -> (PlayerData, Option<ActionMessage>, Vec<Collision2D>) {
    ...
    let can_jump = data.pos.y == 65.0 && data.attack_cooldown <= 0.5 && !is_guard;
    let can_roll = data.pos.y == 65.0 && data.attack_cooldown <= 0.5 && !is_hit && !is_roll;
    let can_guard = data.pos.y == 65.0 && data.attack_cooldown <= 0.5 && data.guard_cooldown <= 0.0 && !is_hit && !is_guard;
    ...
    if is_die || tick == 0 {
        ...
        player_object.bind_mut().gui_update(&data.clone());
    ...
    if roll && can_roll {
        ...
        if dir != 0 {
            data.roll_start_dir = dir;
        } else {
            data.roll_start_dir = anim_player.clone().get_scale().x.signum() as i32;
        }
    }
    if guard && can_guard {
        data.anim_data = Some(PlayAnimationData {
    ...
    data.guard_cooldown = (data.guard_cooldown - delta).max(0.0);
    ...
    for action in actions {
        match action {
            ActionMessage::Attack(attack) => {
                ...
                if is_anim_name(target.clone(), "guard") {
                    target.object.clone().unwrap().bind_mut().show_guard_text();
                    continue;
                }

                let effect = world_data.hit_effect.as_ref().unwrap().instantiate().unwrap();
                let mut effect = effect.clone().cast::<Effect>();
                let mut root_node = target.object.as_ref().unwrap().get_tree().unwrap().get_root().unwrap().get_node("Root".into()).unwrap();
                root_node.add_child(effect.clone().upcast::<Node>());
                effect.set_position(target.pos);
