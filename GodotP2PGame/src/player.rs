...
impl Player {
    ...
    pub fn set_other_player(&mut self) {
        let mut text = self.base().get_node_as::<Label>("MyPlayer");
    ...
    pub fn show_guard_text(&mut self) {
        let mut guard_text = self.base().get_node_as::<Label>("GuardText");
        let mut modulate = guard_text.get_modulate();
        modulate.a = 1.0;
        guard_text.set_modulate(modulate);
    }
    ...
    pub fn gui_update(&mut self, data: &PlayerData) {
        let mut player_health = self.base().get_node_as::<ProgressBar>("ProgressBar");
        player_health.set_value(data.health);
    }
}
...
impl INode2D for Player {
    ...
    fn physics_process(&mut self, delta: f64) {
        ...
        let mut guard_text = self.base().get_node_as::<Label>("GuardText");
        let mut modulate = rollback_text.get_modulate();
        let mut modulate2 = guard_text.get_modulate();

        modulate.a -= 1.0 * delta as f32;
        modulate2.a -= 1.0 * delta as f32;

        rollback_text.set_modulate(modulate);
        guard_text.set_modulate(modulate2);
    }
}
