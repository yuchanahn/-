...
pub fn connect_process(root_node: Gd<Window>, sock: Option<&UdpSocket>, target_addr: SocketAddr) {
    ...
    let game_start_time = timestamp + 3000;
    ...
    *TIMESTAMP_FOR_TIMESYNC.lock().unwrap() = timestamp;
    ...
    fn init(base: Base<Node2D>) -> Self {
        Self {
            ...
            world: WorldData {
                players: HashMap::new(),
                collision: Vec::new(),
                hit_effect: try_load::<PackedScene>("res://Effect/Hit/hit_1.tscn").ok(),
            },
            base,
        }
    }
                ...
                match PacketType::from(buffer[0]) {
                    ...
                    PacketType::Connect => {
                        ...
                        if let Ok(scene) = try_load::<PackedScene>("res://Player/player.tscn") {
                            ...
                            other.bind_mut().set_other_player();
